...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        """
        return ["X", "B-PER", "I-PER", "B-LOC", "I-LOC", "B-ORG", "I-ORG", "O", "[CLS]", "[SEP]"]
...
def create_model(bert_config, is_training, input_ids, input_mask,
    ...
    if FLAGS.use_pos:
        ...
        with open('./data/{}/pos2id.pkl'.format(FLAGS.dataset), 'rb') as rf:
            pos2id = pickle.load(rf)
        num_pos = len(pos2id)
        pos_embeddings = tf.one_hot(pos_ids, num_pos, axis=-1)
        embeddings = tf.concat([embeddings, pos_embeddings], -1)
    ...
    if FLAGS.bilstm or FLAGS.crf:
        ...
        bilstm_crf = BiLSTM_CRF(embedded_chars=embeddings, lstm_size=FLAGS.lstm_size, cell_type=FLAGS.cell, num_layers=FLAGS.num_layers,
                                dropout_rate=FLAGS.dropout_rate, num_labels=num_labels, max_seq_length=max_seq_length, labels=label_ids,
